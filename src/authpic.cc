...
cv::Mat Emx2Mat_U8c(const emxArray_uint8_T *srcImage) {
  cv::Mat dstImage = cv::Mat((int)srcImage->size[1], (int)srcImage->size[0], CV_8U,
                         srcImage->data);
  return dstImage;
}
cv::Mat Emx2Mat_U8(const emxArray_real_T *srcImage) {
  cv::Mat dstImage = cv::Mat((int)srcImage->size[1], (int)srcImage->size[0], CV_64F,
                         srcImage->data);
  return dstImage;
}
...
//    imshow("img1", img1);
    ...
    {
        x_div_y_of_delt_distant.push_back(cv::Point2f(i - j).x / cv::Point2f(i - j).y);
    ...
    cv::Rect rect = cv::Rect(cv::Point(50, 50), cv::Point(img2.cols - 50, img2.rows - 50));
    ...
    if (obj.size() != 0) {
        ...
        if (obj.size() >= 8) {
            ...
            cv::Point2f dx;
            if (obj.size() == 1) {
                dx = cv::Point2f(floor((obj[0] - obj[0]).x), floor((obj[0] - obj[0]).y));
            }
            else
            {
                /*求均值*/
...
class MyLoopBody : public cv::ParallelLoopBody {
 public:
  MyLoopBody(emxArray_uint8_T **image, emxArray_real_T **Gimage_im,
             emxArray_boolean_T **BW_im, emxArray_boolean_T **K, cv::Mat *Gim_mat,
             cv::Mat *bw_im) {
    image_ = image;
