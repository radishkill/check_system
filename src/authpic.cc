...
{
public:
  MyLoopBody(emxArray_uint8_T **image, emxArray_real_T **Gimage_im, emxArray_boolean_T **BW_im,
             emxArray_boolean_T **K, Mat *Gim_mat, Mat *bw_im)
  {
    image_ = image;
...
double AuthPic(cv::Mat &speckle_database, cv::Mat &speckle_auth) {
  ...
  for (int i = 0; i < 2; i++)
  {
    ...
    std::cout << "image1 size : " << image[i]->size[0] << " " << image[i]->size[1] << std::endl;
  }

  // gabor_im(image[0], 8, 45, Gimage_im[0], BW_im[0], K[0]);
  // gabor_im(image[1], 8, 45, Gimage_im[1], BW_im[1], K[1]);
  parallel_for_(Range(0, 2), MyLoopBody(image, Gimage_im, BW_im, K, Gim_mat, bw_im));

  double FHD = hamming(bw_im[0], bw_im[1]);
  if (FHD >= 0.1 && FHD <= 0.25)
  {
