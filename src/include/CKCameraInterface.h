...
#include "CKDeviceDef.h"
		...
		#define CKSDK_API __declspec(dllexport)
	#else
		#define CKSDK_API __declspec(dllimport)
	#endif
#else
	#ifndef __stdcall
		#define __stdcall
...
extern "C" {
  ...
  *      // 叠加十字线等
	...
	*@brief @~chinese 图像捕获的回调函数定义 @~english Callback function definition for image capture */
	typedef void (__stdcall* CAMERA_SNAP_PROC)(HANDLE handle, BYTE *pFrameBuffer, stImageInfo* pFrameInfo, LPVOID lpParam);
	...
	*@brief @~chinese 图像显示的回调函数 @~english Image display callback function */
	typedef void (__stdcall* CAMERA_DISPLAY_PROC)(HANDLE handle, HDC hdc, DisplayWindow *pDisplayWindow, LPVOID lpParam);
	...
	*@return @link #__CK_ERRCODE__ 状态码@endlink
	*@~english
	*@brief Set whether to enable dead pixel correction
	*@param[in] hCamera camera handle, obtained by @link #CameraInit @endlink function
	*@param[in] bEnable TRUE: Enable dead pixel correction FALSE: Disable dead pixel correction
	*@return @link #__CK_ERRCODE__ Status code @endlink
	*/
	CKSDK_API CameraSdkStatus __stdcall CameraSetCorrectDeadPixel(HANDLE hCamera, BOOL bEnable);
	...
	*@brief 获取坏点修正是否开启的状态
	*@param[in] hCamera  相机的句柄，由@link #CameraInit @endlink函数获得
	*@param[in] pbEnable 指针，用于保存是否开启坏点修正的状态
	*@return @link #__CK_ERRCODE__ 状态码@endlink
	*@~english
	*@brief Get the status of dead pixel correction
	*@param[in] hCamera camera handle, obtained by @link #CameraInit @endlink function
	*@param[in] pbEnable pointer, used to save the state of whether to enable dead pixel correction
	*@return @link #__CK_ERRCODE__ Status code @endlink
	*/
	CKSDK_API CameraSdkStatus __stdcall CameraGetCorrectDeadPixel(HANDLE hCamera, BOOL *pbEnable);
	...
	*/
	 ...
	 *@return @link #__CK_ERRCODE__ Status Code @endlink
	 *@note Network packet transfer size ranges from 1500 to 9000
	 *@see CameraGigeSetTransPacketSize
	 */
