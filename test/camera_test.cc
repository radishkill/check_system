...
void test_softtrigger()
{
  ...
  CameraGetResolution(hCamera, &resolution_index);
  DWORD w, h;
  CameraGetOutImageSize(hCamera, &w, &h);
  std::cout << w << " " << h << std::endl;
  tSdkImageResolution resolution;
  CameraGetResolutionForSnap(hCamera, &resolution);
  std::cout << resolution.iHeight << " " << resolution.iWidth << std::endl;
